...
public class BackgroundRunner {
  ...
  public func scheduleBackgroundTasks() throws {
    ...
    guard let interval = config.interval else {
      throw BackgroundRunnerPluginError.invalidRunnerConfig(
        reason: "cannot register background task without a configured interval")
    }
  ...
  public func execute(
    config: RunnerConfig, inputArgs: [String: Any]? = nil, callbackId: String? = nil
  ) throws -> [String: Any]? {
    do {
  ...
  public func dispatchEvent(event: String, inputArgs: [String: Any]?, callbackId: String? = nil)
    throws
  {
  ...
  private func loadRunnerConfig() throws -> RunnerConfig? {
    guard
      let capacitorConfigFileURL = Bundle.main.url(
        forResource: "capacitor.config.json", withExtension: nil)
    else {
    ...
    if let capConfig = JSTypes.coerceDictionaryToJSObject(
      try JSONSerialization.jsonObject(with: capacitorConfigFileData) as? [String: Any])
    {
  ...
  private func initContext(config: RunnerConfig, callbackId: String?) throws -> Context {
    guard
      let srcFileURL = Bundle.main.url(
        forResource: config.src, withExtension: nil, subdirectory: "public")
    else {
