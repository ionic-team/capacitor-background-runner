...
TEST_CASE("FileReader tests", "[context]") {
  auto engine = new Engine();

  std::string context_name = "io.ionic.android_js_engine";
  engine->create_context(context_name);

  std::string basic_file_reader_example =
      "const reader = new FileReader();"
      "const blob = new Blob([\"hello world\"]);"
      "reader.addEventListener(\"load\", (evt) => { console.log(evt.target.result); });"
      "reader.readAsText(blob);";

  engine->execute(context_name, basic_file_reader_example);
  engine->wait_for_jobs();
